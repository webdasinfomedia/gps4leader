...
<h2 class="nav-tab-wrapper">  
	<a class="nav-tab nav-tab-active" href="#"><?php _e('Sugar CRM Report','cart2crm')?></a>    
</h2>
<?php 

if($sugarcrm_conection['error']['number'] == 0)
{
?>
...
if(isset($_REQUEST['cust_report']))
{
	...
	$wp_user_search=$db->getCustomerReport($status,$fromdate,$todate,2);
	if(!empty($wp_user_search))
	{
		$filename='Reports/report.csv';
		$fh = fopen(PATH.$filename, 'w') or die("can't open file");
		$header = Array("Full Name","Email","MobileNo","Billing Address","Shipping Address");
		fputcsv($fh, $header);
...
<table class='widefat picks'>
    ...
    if(isset($_POST['migration_report']))
    {
            ...
            if(!empty($result))
            {
                $filename='reports/report.csv';
                $fh = fopen(PATH.$filename, 'w') or die("can't open file");
                $header = Array("ID","Leadid","Oppid","Acid","Modified date","Created Date");
                fputcsv($fh, $header);
