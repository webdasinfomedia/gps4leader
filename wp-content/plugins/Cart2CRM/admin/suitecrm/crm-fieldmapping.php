...
if(isset($_REQUEST['sysnc']))
{
	...
	{
		$table_name = "cart2crm_leads";
		$leads_data=$db->getLeadsData($table_name,3);
		$array_data1 =array();
		$fetch_all_data=$db->getExistleadProductId(3);
		...
		if(!empty($fetch_all_data))
		{
			foreach($fetch_all_data as $post)
			{
				...
				foreach($post_meta as $metadata){
					...
					$array_data1[$post->id]['order_id'] = $post->id;
				}
				...
				if(!$db->is_orderlead_exist($post->id,3))
				$wp_in=$db->addLeadsCustomCRM($post->id,1,$cldate,3,$result['id']);
	...
	if($current_tab == 'Opportunity')
	{
		...
		$leads_data=$db->getLeadsData($table_name,3);
		$array_data1 =array();
		$fetch_all_data=$db->getExistopportunityProductId(3);
		...
		if(!empty($fetch_all_data))
		{
			foreach($fetch_all_data as $post)
			{
				...
				if(!$db->is_orderopportunity_exist($post->id,3))
				{
					$wp_in=$db->addOpportunityCustomCRM($post->id,1,$cldate,3,$result['id']);
	...
	if($current_tab == 'Account')
	{
		...
		$leads_data=$db->getLeadsData($table_name,3);
		$array_data1 =array();
		$fetch_all_data=$db->getExistaccountProductId(3);
		...
		if(!empty($fetch_all_data))
		{
			...
			{	
				...
				$cldate = date('Y-m-d H:i:s');	
					if(!$db->is_orderaccount_exist($post->id,3))
				{
					$wp_in=$db->addaccountCustomCRM($post->id,1,$cldate,3,$result['id']);
				}
...
<h2 class="nav-tab-wrapper">  
	<a class="nav-tab nav-tab-active" href="#"><?php _e('SuiteCRM Field Mapping','cart2crm')?></a>    
</h2>
<?php 
    ...
    <a href="?page=crm_field_mapping&tab=Account" class="nav-tab <?php echo $active_tab == 'Account' ? 'nav-tab-active' : ''; ?>">Account</a>  
     ...
     <input type="hidden" name = "current_tab" value = "<?php echo $active_tab;?>">
     	...
     	<table class="customformlead">
     		<tbody>
     			<tr valign="top">
     				<td class="title"><?php _e('Woocommerce','cart2crm');?>dede</td>
     				<td><?php global $wpdb;
					...
					<td class="title"><?php _e('SuiteCRM','cart2crm');?></td>
					<td>
        ...
        <table class="widefat" style="margin-top:5px">
	        ...
	        <tbody>
		        <tr>
			        ...
			        <th><?php _e('SuiteCRM Fields','cart2crm');?></th>
			       
			        <th><?php _e('Delete','cart2crm');?></th>
		        </tr>	
		      <?php 
		        	$table_name = "cart2crm_opportunity";
		        	$record = $db->getcrmData($table_name,3);
	        ...
	        <thead>
	        <tr>
	        	<th colspan="3"><?php _e('Accounts fields','cart2crm');?></th>
	        </tr>
	        </thead>
	        <tbody>
		        <tr>
