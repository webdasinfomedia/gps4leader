...
$user_auth = array(
	...
	if($_REQUEST['confirm_order'] == 'on' && $_REQUEST['aband_cart'] == "on")
	{
		...
		echo  "<div style='background: none repeat scroll 0 0 green;color: white;float: left;font-size: 17px;margin-top: 35px;padding: 10px;width: 98%; margin-bottom: 10px;'>Syncronize your data sucessfully</div>";
	}
	elseif ($_REQUEST['confirm_order'] == 'off' && $_REQUEST['aband_cart'] == "on")
	{
		...
		$opportunity_data=$db->getLeadsData('cart2crm_opportunity',3);
		$account_data=$db->getLeadsData('cart2crm_account',3);
		$wp_user_search=$db->getOnoffIntegration('on-hold',3);
		foreach ( $wp_user_search as $post ) {
			$post_meta=$db->getPostmeta($post->id);
			foreach($post_meta as $metadata){
				$array_data1[$post->id][$metadata->meta_key] = $metadata->meta_value;
			...
			$cldate = date('Y-m-d H:i:s');
			$wp_in=$db->addCRMID($post->id,$ls,$op,$ac,$date,$cldate,3);
		
			 
		}
		echo  "<div style='background: none repeat scroll 0 0 green;color: white;float: left;font-size: 17px;margin-top: 35px;padding: 10px;width: 98%;'>Syncronize your data sucessfully</div>";
	}
	elseif ($_REQUEST['confirm_order'] == 'on' && $_REQUEST['aband_cart'] == "off")
	{
		...
		$opportunity_data=$db->getLeadsData('cart2crm_opportunity',3);
		$account_data=$db->getLeadsData('cart2crm_account',3);
	$wp_user_search=$db->getOnoffIntegration('completed',3);
...
<h2 class="nav-tab-wrapper">  
	<a class="nav-tab nav-tab-active" href="#"><?php _e('Suite CRM Integarion','cart2crm')?></a>    
</h2>
<div class="pick">
<form method="post" name="ordst">
<h3 class="order">
<p title="<?php _e('A WooCommerce order is created (I.e. a sale): Ability to turn this transfer of data between WooCommerce and SuiteCRM On/Off.','cart2crm');?>">
<?php _e('Confirmed Order Sync','cart2crm');?> :</p>
<?php $imgurl=plugins_url( 'Cart2CRM/assets/images/');?>
	<input type="hidden" id="confirm_order1" name="confirm_order" value="<?php if(isset($suitecrm_options['confirm_order'])) echo $suitecrm_options['confirm_order']; else echo "on";?>">
		<img style="cursor: pointer; cursor: hand;" 
		src="<?php if(isset($suitecrm_options['confirm_order'])) echo plugins_url( 'Cart2CRM/assets/images/'.$suitecrm_options['confirm_order'].'.png'); 
		else echo plugins_url( 'Cart2CRM/assets/images/on.png');?>" id="suite_confirm_order" customurl="<?php echo $imgurl;?>">

</h3>
<h3 class="cart">
<p title="<?php _e('A WooCommerce abandoned cart occurs (I.e. no sale): Ability to turn this transfer of data between WooCommerce and SuiteCRM On/Off.','cart2crm');?>">
<?php _e('Abandoned Cart Sync','cart2crm');?> :</p>
<input id="aband_cart" type="hidden" name="aband_cart" value ="<?php if(isset($suitecrm_options['aband_cart'])) echo $suitecrm_options['aband_cart']; else echo "on";?>">
<img id="suite_aband_cartimg" style="cursor: pointer; cursor: hand;"  customurl="<?php echo $imgurl;?>"
src="<?php if(isset($suitecrm_options['aband_cart'])) echo plugins_url( 'Cart2CRM/assets/images/'.$suitecrm_options['aband_cart'].'.png'); else echo plugins_url( 'Cart2CRM/assets/images/on.png');?>">
